...
!(async () => {
    if (!cookiesArr[0]) {
        $.msg($.name, "【提示】请先获取cookie\n直接使用NobyDa的JD签到获取", "https://bean.m.jd.com/", {
...
/**
 * 获取分享码
 * @param {String} cookie JDcookie字符串
 */
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.data.shareTaskRes) {
                            ...
                            $.shareCode.jdzz.err = `已满5人助力,暂时看不到您的JD赚赚好友助力码`;
                        }
                    }
                }
            } catch (e) {
...
async function shareCode_jdCash() {
    var request = {
        url: `${JD_API_HOST}?functionId=cash_mob_home&body=${escape(
            JSON.stringify({})
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 0 && data.data.result) {
                            $.shareCode.jdcash.code = data.data.result.inviteCode;
                        } else {
                            $.shareCode.jdcash.err = "未能成功获取数据";
                        }
                    }
                }
            } catch (e) {
...
function safeGet(data) {
    try {
        ...
        console.log(e);
        console.log(`JD服务器访问数据为空，请检查自身设备网络情况`);
        return false;
    }
}
...
function msgShow() {
    return new Promise(async (resolve) => {
        ...
        // $.msg($.name, ``, `【JD账号${$.index}】${$.UserName}${shareCodeMsg}`);
        if ($.isNode()) {
            await notify.sendNotify(
                `${$.name} - 账号${$.index} - ${$.UserName}`,
                `【JD账号${$.index}】${$.UserName}${shareCodeMsg}`
            );
        }
        resolve();
    });
...
/** 生成Bot需要的数据 */
async function TuringLabbot() {
    return new Promise(async (resolve) => {
        let bean = `/submit_bean_code `;
        let farm = `/submit_farm_code `;
        let pet = `/submit_pet_code `;
        let jxfactory = `/submit_jxfactory_code `;
        let ddfactory = `/submit_ddfactory_code `;
        for (const shareCode of shareCodes) {
            if (shareCode.bean.code) bean += `${shareCode.bean.code}&`;
            if (shareCode.farm.code) farm += `${shareCode.farm.code}&`;
            if (shareCode.pet.code) pet += `${shareCode.pet.code}&`;
            if (shareCode.jxfactory.code) jxfactory += `${shareCode.jxfactory.code}&`;
            if (shareCode.jdfactory.code) ddfactory += `${shareCode.jdfactory.code}&`;
        }
        let result = `【可提交的互助码】- @TuringLabBot
${bean.substring(0, bean.length - 1)}

${farm.substring(0, farm.length - 1)}

${pet.substring(0, pet.length - 1)}

${jxfactory.substring(0, jxfactory.length - 1)}

${ddfactory.substring(0, ddfactory.length - 1)}`;
        // $.msg($.name, ``, result);
        if ($.isNode()) await notify.sendNotify($.name, encodeURIComponent(result));
        resolve();
    });
}

async function CommitCodeBot() {
    return new Promise(async (resolve) => {
        let jdcrazyjoy = `/jdcrazyjoy `;
        let jdzz = `/jdzz `;
        let jdcash = `/jdcash `;
        // let jdnh = `/jdnh `;
        // let jdnian = `/jdnian `;
        for (const shareCode of shareCodes) {
            if (shareCode.crazyjoy.code) jdcrazyjoy += `${shareCode.crazyjoy.code}&`;
            if (shareCode.jdzz.code) jdzz += `${shareCode.jdzz.code}&`;
            if (shareCode.jdcash.code) jdcash += `${shareCode.jdcash.code}&`;
            // if (shareCode.jxfactory.code) jdnh += `${shareCode.jxfactory.code}&`;
            // if (shareCode.jdfactory.code) jdnian += `${shareCode.jdfactory.code}&`;
        }
        let result = `【可提交的互助码】- @LvanLamCommitCodeBot
${jdcrazyjoy.substring(0, jdcrazyjoy.length - 1)}

${jdzz.substring(0, jdzz.length - 1)}

${jdcash.substring(0, jdcash.length - 1)}`;
        // $.msg($.name, ``, result);
        if ($.isNode()) await notify.sendNotify($.name, encodeURIComponent(result));
        resolve();
    });
